...
public class RedisTagManagerIntegrationTests : RedisIntegrationTestBase
{
    ...
    public async Task InvalidateByTagsAsync_ShouldRemoveTaggedEntries()
    {
        ...
        await CacheManager.GetOrCreateAsync("GetProfile", new object[] { 123 }, () => Task.FromResult("Profile Data 123"), settings1, keyGenerator, false);
        await CacheManager.GetOrCreateAsync("GetProfile", new object[] { 456 }, () => Task.FromResult("Profile Data 456"), settings2, keyGenerator, false);
        await CacheManager.GetOrCreateAsync("GetSettings", new object[] { 123 }, () => Task.FromResult("Settings Data 123"), settings3, keyGenerator, false);
        ...
        var callCount = 0;
        var profile123 = await CacheManager.GetOrCreateAsync("GetProfile", new object[] { 123 }, () => { callCount++; return Task.FromResult("New Profile 123"); }, settings1, keyGenerator, false);
        var profile456 = await CacheManager.GetOrCreateAsync("GetProfile", new object[] { 456 }, () => { callCount++; return Task.FromResult("New Profile 456"); }, settings2, keyGenerator, false);
        var settings123 = await CacheManager.GetOrCreateAsync("GetSettings", new object[] { 123 }, () => { callCount++; return Task.FromResult("New Settings 123"); }, settings3, keyGenerator, false);
    ...
    public async Task InvalidateByTagsAsync_WithMultipleTags_ShouldInvalidateAllMatchingEntries()
    {
        ...
        await CacheManager.GetOrCreateAsync("GetData", new object[] { 1 }, () => Task.FromResult("value1"), settings1, keyGenerator, false);
        await CacheManager.GetOrCreateAsync("GetData", new object[] { 2 }, () => Task.FromResult("value2"), settings2, keyGenerator, false);
        await CacheManager.GetOrCreateAsync("GetData", new object[] { 3 }, () => Task.FromResult("value3"), settings3, keyGenerator, false);
        ...
        // Assert
        var callCount = 0;
        var value1 = await CacheManager.GetOrCreateAsync("GetData", new object[] { 1 }, () => { callCount++; return Task.FromResult("new1"); }, settings1, keyGenerator, false);
        var value2 = await CacheManager.GetOrCreateAsync("GetData", new object[] { 2 }, () => { callCount++; return Task.FromResult("new2"); }, settings2, keyGenerator, false);
        var value3 = await CacheManager.GetOrCreateAsync("GetData", new object[] { 3 }, () => { callCount++; return Task.FromResult("new3"); }, settings3, keyGenerator, false);
    ...
    public async Task InvalidateByTagsAsync_WithNonExistentTag_ShouldNotAffectOtherEntries()
    {
        ...
        await CacheManager.GetOrCreateAsync("TestMethod", new object[] { "test" }, () => Task.FromResult("test-value"), settings, keyGenerator, false);
        ...
        // Assert
        var callCount = 0;
        var value = await CacheManager.GetOrCreateAsync("TestMethod", new object[] { "test" }, () => { callCount++; return Task.FromResult("new-value"); }, settings, keyGenerator, false);
